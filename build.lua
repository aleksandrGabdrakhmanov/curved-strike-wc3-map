...
local function folderFileList(pathFolder, outTable)
	...
	-- find = Unix command to list files
	-- pathFolder = directory path
	-- -type f = list files only
	local findCmd = [[find "]] .. pathFolder .. [[" -type f | sort]]
	local findErrorIndex = #outTable + 1 -- first line
	for item in io.popen(findCmd):lines() do
		table.insert(outTable, item)
	end

	-- Check if the last entry is an error message
	local lastEntry = outTable[#outTable]
	if lastEntry and lastEntry:find("cannot", 1, true) then
		return false, assert(lastEntry)
	else
		return outTable
	end
end
